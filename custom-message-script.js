...
db.query('SELECT id,content FROM "custom_messages"').spread((response, metadata) => {
	if(response.length>0){
		response.forEach(function (item,indxx) {
			console.log('---------------',item);
			var query2 = "UPDATE custom_messages SET content_new='"+item.content+"' WHERE id = '"+item.id+"'";
