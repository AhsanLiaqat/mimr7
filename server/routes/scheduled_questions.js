...
var j = schedule.scheduleJob('01 * * * * *', function(){
    model.scheduled_question.findAll({
        ...
        questions.forEach(function(msg) {
            if(new Date(msg.setOffTime).getFullYear() == new Date().getFullYear() &&
               ...
               new Date(msg.setOffTime).getMinutes() == new Date().getMinutes()
                ...
                model.scheduled_question.findOne({
                    where: {id: msg.id},
                    include: [
                    {
                        model : model.user
                    ...
                    var link = "\n\n\n\n\n\n\n\n\n\n\n\n"+'http://app.mimr7.com/#/pages/content-questions' + '/' + scheduled_question.userId + '/' + scheduled_question.id;
                ...
                model.scheduled_question.update({activated: true, activatedAt: new Date()}, {where: {id: msg.id}})
        ...
        questions.forEach(function(msg) {
            if(new Date(msg.activatedAt.getTime() + msg.total_time*1000).getFullYear() == new Date().getFullYear() &&
               ...
               new Date(msg.activatedAt.getTime() + msg.total_time*1000).getSeconds() == new Date().getSeconds()
                ...
                model.scheduled_question.update({status: true}, {where: {id: msg.id}})
                .then(function(gamePlanTmplate) {
                    model.scheduled_question.findOne({
                        where: {id: msg.id},
                        include: [{
...
router.post('/update-message-off-set/:id',function(req,res,next){
    ...
    model.scheduled_question.findOne({
        ...
        model.scheduled_question.update(record,{where: { id : req.params.id }})
        .then(function(result) {
            ...
            // }).then(function(scheduled_question) {
            //     // _.each(scheduled_question.content_plan_template.class_list.users, function (user){
...
router.post('/send-question/:id',function(req,res,next){
    model.scheduled_question.update({activated: true, activatedAt: new Date()}, {where: {id: req.params.id}})
        .then(function(gamePlanTmplate) {
        model.scheduled_question.findOne({
            where: {id: req.params.id},
            include: [{
...
router.post('/skip/:id',function(req,res,next){
    model.scheduled_question.update(req.body.data,{where: { id : req.params.id }})
    .then(function(result) {
        res.send(result);
    });
...
router.post('/save', function(req, res, next) {
    ...
    model.scheduled_question.create(record)
        .then(function(msg) {
            res.send(msg);
        });
});
