...
(function () {
    ...
    angular.module('app')
        .controller('simpleScheduledQuestionCtrl', ['$timeout', '$scope', '$routeParams', '$http', 'AuthService', 'Query', 'filterFilter','ModalService','$sce','$uibModal', addFunction]);
    ...
    function addFunction($timeout, $scope, $routeParams, $http, AuthService, Query, filterFilter,ModalService,$sce,$uibModal,uiCalendarConfig) {
        ...
        $scope.init = () => {
            ...
            $http.get('questions/get/' + $routeParams.contentId).then(function(response){
                $scope.questions = response.data;
                console.log('---',$scope.questions)
                angular.forEach($scope.questions , function(question,qIndex){
                    if(question.offset){
                        // $scope.events.push({
                        //     id: question.id, 
                        ...
                        $timeout(function() {
                            ...
                            $('#'+qIndex).draggable({
                                    ...
                                    drag: function() {
                                        $(this).parent().parent().parent().parent().parent().css('overflow','inherit');
                                        $(this).parent().parent().parent().css('overflow' , 'inherit');
                                        $(this).parent().css('overflow' , 'inherit');
                                        console.log('---',$(this).closest('.questions-wrapper'));
                                    },
                                    stop: function() {
                                        $(this).parent().parent().parent().parent().parent().css('overflow','auto');
        ...
        $scope.save = function () {
            if($scope.data.id){
                $scope.data.articleId = $routeParams.contentId;
