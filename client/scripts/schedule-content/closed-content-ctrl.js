(function () {
    ...
    angular.module('app')
        .controller('closedContentsCtrl', ['$scope', '$filter', '$routeParams', '$http', 'AuthService', 'ModalService', '$location', gameRoleCtrl]);
    ...
    function gameRoleCtrl($scope, $filter, $routeParams, $http, AuthService, ModalService, $location) {
        ...
        $scope.pageItems = 10;
        $scope.contentTable = function (tableState) {
            $scope.tableState = tableState;
            $scope.isLoading = true;
            $http.get('/content-plan-templates/closed-contents')
            .then(function (response) {
                $scope.plan_contents = response.data;
                $scope.managearray();
            });
        };
