    ...
    function dashCtrl($q,$scope ,$http ,$rootScope ,$route,AuthService ,$routeParams,ModalService ,$location ,$timeout,filterFilter ,$filter ,$window,Query, AllCategoryService, ActivityService, TaskService, ActionPlanService, ClassService, MessageHistoryService, MessageService, PlanActivityService, SectionService, AgendaPointService) {
        ...
        $scope.DragOptionsTaskList = {
            ...
            itemMoved: function (event) {
                ...
                console.log('agendaPoint created',sectionId)
                if($scope.actionPlan.kind == 'activities'){
                    if (sectionId) {
                    ...
                    angular.forEach(event.dest.sortableScope.modelValue, function(obj, ind) {
                        if(obj.name == task.title){
                            found = true; 
                        }
                    });
                    if(found){
                        toastr.warning('Task Already associated in this AgendaPoint');
                    ...
                    if(!found){
                        ...
                        var activity= {
                            ...
                            ActivityService.create(act).then(function(resp){
                                var data = {
                                    type: 'action',
                                    agendaPointId : sectionId,
                                    name : task.title,
                                    responseActorId: task.userAccountId,
                                    activityId: resp.data.id
                                }
                                $http.post('/settings/agenda-activities/save', { data: data }).then(function (res) {
        ...
        $scope.addIncidentAgendaActivities = function(agendaPoint,catIndex,agendaPointIndex,event) {
            ...
            ModalService.showModal({
                ...
                modal.close.then(function(result) {
                    if(result){
                        $scope.actionPlan.categories[catIndex].agendaPoints[agendaPointIndex] = result;
                    }
                    $('.modal-backdrop').remove();
                    $('body').removeClass('modal-open');
                });
        ...
        $scope.changeActionPlan = function (id) {
            ...
            if($scope.actionPlan.kind == 'activities'){
                ...
                ActionPlanService.getAgendaPoints($scope.actionPlanId).then(function(response){
                    if(response.data){
                        $scope.actionPlan.agendaPoints = response.data.agendaPoints;
                        ActionPlanService.getAllCategoryAgendaPoints($scope.actionPlanId).then(function(response){
                            $scope.actionPlan.categories = response.data;  
                        })
                        // var points=[];
                        // angular.forEach($scope.actionPlan.agendaPoints , function(agendaPoint, index){
                        //     if(agendaPoint.agenda_activities.length > 0){
                        //         agendaPoint.agenda_activities = _.sortBy(agendaPoint.agenda_activities, function (act) { return act.index });
                        //     }
                        //     points.push(angular.copy(agendaPoint))
                        // });
                    }
                    // var catgry = [];
                    ...
                    // });
                    // console.log('->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>',catgry);
                    // $scope.actionPlan.categories = catgry;
                },function(err){
                    if(err)
                        toastr.error(AppConstant.GENERAL_ERROR_MSG,'Error')
                    else
        ...
        $scope.createActionPlanCopy = function () {
            ModalService.showModal({
                templateUrl: "views/crisis-manager/planning-dashboard-v2/new-action-plan-copy.html",
                controller: "newActionPlanCopyModalCtrl"
            }).then(function (modal) {
                modal.element.modal({ backdrop: 'static', keyboard: false });
                modal.close.then(function (result) {
                    if (result) {
        ...
        $scope.updateAgendaActivityActivation = function (activity) {
            ...
            setTimeout(function () {
                $http.post("/settings/agenda-activities/update", activity)
        ...
        $scope.getStatusName = function (status) {
            return (status)? filterFilter($scope.statusOptions, { 'value': status })[0].name : 'NO INFORMATION';
        }
            ...
            ModalService.showModal({
                ...
                modal.close.then(function (response) {
                    ...
                    if (response && response !== '') {
                        $scope.actionPlan.categories[cat_index].agendaPoints[agenda_index].agenda_activities[act_index] = response;
                    }
                });
            });
        };
        $scope.removeIncidentAgendaActivities = function(agendaPoint,catIndex,agendaPointIndex,event){
            ...
            if(confirm('Are you sure?')){
                angular.forEach(agendaPoint.agenda_activities, function(activity){
                    $http.delete("/settings/agenda-activities/delete/" + activity.id)
                    .then(function (result) {});
                });
                var left = $scope.actionPlan.agendaPoints.filter(function(x) { return x.id != agendaPoint.id; }).map(function(item){ return item.id});
                ActionPlanService.setAgendaPointList({data : $scope.actionPlan.id, selected : left}).then(function(response){
                    $scope.actionPlan.categories[catIndex].agendaPoints.splice(agendaPointIndex,1);
                    // if($scope.actionPlan.categories[catIndex].data.length == 0){
                    //     $scope.actionPlan.categories(catIndex,1)
                    // }
                },function(err){
                    if(err)
                        toastr.error(AppConstant.GENERAL_ERROR_MSG,'Error')
        ...
        $scope.addIncidentAgendaPoint = function(category,ind){
            ModalService.showModal({
                ...
                inputs : {
                    ...
                    incident_plan_id: null,
                    agenda_point: null,
                    activityType : 'agenda',
                    action_plan_id: $scope.actionPlan.id
                }
            }).then(function(modal) {
                modal.element.modal( {backdrop: 'static',  keyboard: false });
                modal.close.then(function(result) {
                    if(result){
                        $scope.actionPlan.categories[ind].agendaPoints.push(result);
                    }
                    $('.modal-backdrop').remove();
                    $('body').removeClass('modal-open');
                });
