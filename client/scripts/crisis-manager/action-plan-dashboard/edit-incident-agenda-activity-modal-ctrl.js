(function () {
    ...
    angular.module('app')
    .controller('editIncidentAgendaActivityModalCtrl', ['$scope',
    '$rootScope',
    'close',
    '$routeParams',
    '$http',
    ...
    function ctrlFunction($scope,
        ...
        activityType) {
            ...
            function init() {
                ...
                var url = (activityType == 'incident-agenda')?'/incident-agenda-activities/':'/settings/agenda-activities/';
                url +='get?id='+activityId;
                $http.get(url)
            ...
            $scope.update = function () {
                if (validateActivity()) {
                    if(activityType == 'incident-agenda'){
                        ...
                        $scope.activity.name = filterFilter($scope.tasks, { 'id': $scope.activity.activity.taskListId })[0].title;
                        var data = { activity: $scope.activity.activity, outcomes: [] };
                        ActivityService.update(data).then(function(response){
                            $http.post('/settings/agenda-activities/update', $scope.activity)
                            .then(function (response) {
                                toastr.success("Activity updated", 'Success!');
                                $scope.close(response.data);
                            });
